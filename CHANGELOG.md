...
- 5번 제시카 누락 문장·페이지 의미 복구, 6번 전체 125항목 검수/54항목 교정.
- 셀렉트 38·39번 120/110항목 검수와 30/32항목 교정, 엘윈 이라면/이다 반영.
- 엘윈 조사 527문맥 검사, 최근 검수 외 15챕터 2,322행 추가 원문 대조,
  원문 동일 분기 복제 74~78 번역 복구. 전체 98리소스 12,591행 구조 검사.
- 워해머 폐기 뒤 잔여 글자와 그레이트 소드 폐기창 테두리 깨짐 수정.
- 로우가 습득 마법명 증 → 참: 이름표 108개 수정, 습득 조건·마법 효과 보존.
- 50번 및 관련 존대·문맥 교정, 잘 가 제시카 → 안녕히 가세요 제시카 님,
  사부님 → 스승님 통일, 원망할 거라면 줄바꿈 정리.
- 제국/어둠 엔딩의 별도 대사 18개 번역·편집기 분류 추가. 제국 엔딩 실제 진행 확인.
- MP 부족 안내 인코딩 수정. 해당 실플레이 검증은 사용자 요청으로 생략.
- 수정 내역과 무편집 이미지 35장 추가. 기존 엔딩 검은 화면 문제는 미해결.

[전체 수정 내역·이미지](docs/RELEASE_V0.97.md) · [검증 한계](docs/VERIFICATION_V0.97.md).
V0.96 및 이전 릴리스·태그·파일은 변경하지 않습니다.
