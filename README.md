...
The current public version is **v0.825**. It is a development/pre-release
snapshot, not a final-completion claim. This update includes automatic-movie
subtitles, Gel Gather labels, victory-condition menu boundaries, Muscle Temple
dialogue and 12×12 Hangul name glyph fixes. The new Muscle Temple wording still
requires full human review. See the [v0.825 release notes](docs/RELEASE_v0.825.md)
...
Download the automatic patcher from [GitHub Releases](https://github.com/OldManCraveGuitars-bit/langrisser-fx-korean-patch/releases/tag/v0.825).
...
The new hidden Muscle Temple resource adds 86 nonempty dialogue records and one
empty slot outside that historical Scenarios 1–70 editor catalog. These counts
describe implemented populations, not percentages or complete human/hardware QA.
...
Run `Langrisser-FX-KR-Auto-Patcher-v0.825.exe`, select the original Japanese CUE, and
...
```powershell
python patch/apply_patch.py "path/to/original Track 2.bin" "Track-2.KR.bin" --patch patch/Langrisser-FX-KR-v0.825.lfxpatch
```
...
v0.825 display, subtitle lifecycle, condition-boundary and font-policy modules.
...
- The earlier cumulative tool baseline was not fully green: 8 known failures and 1 known error were recorded. Their resolution is not claimed by v0.825.
- The new Muscle Temple dialogue passes technical checks, but full human wording review and all hidden branches remain unfinished.
- An all-scenario playthrough and physical-console/iPhone verification are not recorded for the complete v0.825 scope.
- The final target was checked through automatic openings 1/2, hidden-scenario introduction, condition menus, shop and native combat. It was not played through every natural movie trigger.
...
현재 공개 버전은 **v0.825**이며 완성판이 아니라 개발/사전 공개
후보입니다. 자동 영상 자막, 겔 게더/겔게더 표기, 승리조건 메뉴,
근육의 신전 대사와 12×12 이름 글꼴 수정을 포함합니다.
근육의 신전 새 번역은 기술 검증을 통과했지만 전체 문장 검수는 진행 중입니다.
[수정 내역과 검증 범위](docs/RELEASE_v0.825.md)를 확인하세요.
이전 누적 도구 검사에서 기록된 실패 8개와 오류 1개의 해결이나 모든 시나리오
완주 검증을 주장하는 업데이트는 아닙니다.

v0.825는 **원본 일본판에 새로 적용하는 누적 패치**입니다. 이전 한글판에
